...
select count(distinct ifnull(dept_name, 'a')) from emp; -- null값 포함한 개수. null을 실제 값을 바꿔서 카운트.
...
from emp
where char_length(emp_name) = (select max(char_length(emp_name)) from emp);
...
select if(salary < 10000, "만 미만", "만 이상") as '급여',
	   count(*) as '직원수'
from emp
