...
-- EMP 테이블에서 salary에 '$'를 붙여 문자열로 출력
...
select emp_id,
       ...
       ceil(salary * 1.15) - salary         # 설정한 별칭을 다른 변수로 사용 못함
from emp;
...
select adddate(curdate(), interval -10 day) as "입사 10일 전",
	   hire_date,
	   adddate(curdate(), interval 10 day) as "입사 10일 후"
from emp
where dept_name = 'IT';
...
select emp_id,
	   emp_name,
       salary,
       ifnull(round(salary * comm_pct,0), 0) as 'comm'
from emp;
...
select emp_id,
       ...
       case job when 'AD_PRES' then "대표"
                ...
                when 'IT_PROG' then job     -- 조회된 값이 그대로 출력 >> 컬럼명 지정.
				end as "job"
from emp
